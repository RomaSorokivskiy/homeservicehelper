...
export async function GET() {
  ...
  const home = {
    ...
    controls: haItems.filter((x) => /^(light|switch)\.[a-z0-9_]+$/.test(String(x.entity_id))).map((x) => ({ id: x.entity_id, name: (x.attributes as Record<string,unknown>|undefined)?.friendly_name || x.entity_id, on: x.state === "on", domain: String(x.entity_id).split(".")[0] })).slice(0,12),
  };
